...
class Auto21Model(BaseModel):
    ...
    def backward_G11(self, epoch1, batch, lstart):
        ...
        #print("shape of grad :", np.shape(self.grad))   
        if (epoch1 % 10 == 0): 
            filen = './marmousi21/GradNADR4' + str(batch)+'ep'+str(epoch1)+'.npy' #switch on for physics based fwi       
            np.save(filen, self.grad.cpu().detach().numpy())  #switch on physics based fwi
        
            filen = './marmousi21/FakeNADR4' + str(batch)+'ep'+str(epoch1)+'.npy' #switch on for physics based fwi       
            np.save(filen, self.fake_B.cpu().detach().numpy())  #switch on physics based fwi
            
            filen = './marmousi21/RealNADR4' + str(batch)+'ep'+str(epoch1)+'.npy' #switch on for physics based fwi       
