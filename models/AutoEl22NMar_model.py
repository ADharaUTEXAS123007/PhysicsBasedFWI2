...
class AutoEl22NMarModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm='batch', netG='AutoEl22NMar',
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizer_G1 = torch.optim.Adam(
                [self.netG.modelVp], lr=0.0)
            self.optimizer_G2 = torch.optim.Adam(
                [self.netG.modelVs], lr=0.0)
            self.optimizer_G3 = torch.optim.Adam(
                [self.netG.modelRho], lr=0.0)
