...
class AutoElFullMar22Model(BaseModel):
    ...
    def backward_G11(self, epoch1, batch, lstart, initerror, currenterror, idx):
        ...
        #self.grad = self.grad/torch.max(self.grad.abs())
            ...
            #################
            self.loss_G.backward()
            ####2####self.vp_grad = torch.unsqueeze(self.vp_grad,0)
            ####2####self.vp_grad = self.vp_grad.cuda(self.fake_Vp.get_device())
            ####2####self.fake_Vp.backward(self.vp_grad,retain_graph=True)
            #self.fake_Vs.retain_grad()
            #self.fake_Vp.retain_grad()
            #self.fake_Rho.retain_grad()
            
            ####2####self.vs_grad = torch.unsqueeze(self.vs_grad,0)
            ####2####self.vs_grad = self.vs_grad.cuda(self.fake_Vs.get_device())
            ####2####self.fake_Vs.backward(self.vs_grad,retain_graph=True)
            #self.fake_Vs.retain_grad()
            #self.fake_Vp.retain_grad()
            #self.fake_Rho.retain_grad()
