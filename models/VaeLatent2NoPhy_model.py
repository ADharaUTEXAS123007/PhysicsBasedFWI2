...
class VaeLatent2NoPhyModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm='batch', netG='VaeLatent2NoPhy',
    ...
    def backward_G11(self, epoch1, batch, lstart):
        ...
        self.loss_M_MSE = self.criterionMSE(self.real_A, self.fake_B)*1200/(diff_size[0]*diff_size[1]*diff_size[2]*diff_size[3])
        #k
        kld_loss = torch.mean(-0.5 * torch.sum(1 + self.log_var - self.mu ** 2 - self.log_var.exp(), dim = 1), dim = 0)
    ...
    def compute_loss_only(self):
        ...
        diff_size = self.real_B.size()
        lossMSE = self.criterionMSE(self.fake_BT, self.real_A)*800/(diff_size[0]*diff_size[1]*diff_size[2]*diff_size[3])
