...
class AutoElFullRhoMar22Model(BaseModel):
    ...
    def backward_G11(self, epoch1, batch, lstart, initerror, currenterror, idx):
        ...
        #self.grad = self.grad/torch.max(self.grad.abs())
            ...
            ###555#### self.rho_grad = torch.unsqueeze(self.rho_grad,0)
            ###555### self.rho_grad = self.rho_grad.cuda(self.fake_Rho.get_device())
            ######555### self.fake_Rho.backward(self.rho_grad)
