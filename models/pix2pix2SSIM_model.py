...
class Pix2Pix2SSIMModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm='batch', netG='unet_128', dataset_mode='unaligned2', ngf='32')
        if is_train:
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.ssim_loss = pytorch_ssim.SSIM(window_size=5)
        else:
            print("----test data----")
            self.criterionL1 = torch.nn.L1Loss()
            self.criterionRMSE = torch.nn.MSELoss()
    ...
    def compute_loss_only(self):
        lossL1 = self.criterionL1(self.fake_BT,self.real_BT)*100
        ...
        ssim_loss = self.ssim_loss(self.fake_BT, self.real_BT)
        print("ssim_loss : " + str(ssim_loss.cpu().float().numpy()))
