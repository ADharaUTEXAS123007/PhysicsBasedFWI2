...
class AutoSEAMMar22_Net(nn.Module):
    ...
    def prop(self, vp1, vs1, rho1, true, epoch1, freq, idx, it, nnz):
        ...
        #d.forward(model, src, rec)
        #os.system('mpirun -np 4 hello')
        filen = './marmousiSEAM9July/vpmodS2EG' + str(epoch1) + '.npy' #switch on for physics based fwi         
        np.save(filen, vpst)  #switch on physics based fwi
        
        filen = './marmousiSEAM9July/vsmodS2EG' + str(epoch1) + '.npy' #switch on for physics based fwi     
        np.save(filen, vsst)  #switch on physics based fwi
        
        filen = './marmousiSEAM9July/rhomodS2EG' + str(epoch1) + '.npy' #switch on for physics based fwi     
        np.save(filen, rhost)  #switch on physics based fwi
        ...
        #d.add_fwi_stage(fc_low=0.0, fc_high=int(epoch1/10)+1.0)
        #d.add_fwi_stage(fc_low=0.0, fc_high=30.0)
        d.add_fwi_stage(fc_low=2.0, fc_high=10, inv_rho_iter=10000, lnorm=2)
        # if ((epoch1 >= 0) and (epoch1 <=100 )):
        #     d.add_fwi_stage(fc_low=0.0, fc_high=2.0)
        # #     #print(f'Stage {i+1}:\n\t{d.fwi_stages[i]}\n')
        # elif ((epoch1 >= 101) and (epoch1 <=200)):
        ...
        filen = './marmousiSEAM9July/vppS2EG' + str(epoch1) + '.npy' #switch on for physics based fwi       
        np.save(filen, vp_grad)  #switch on physics based fwi
        
        filen = './marmousiSEAM9July/vssS2EG' + str(epoch1) + '.npy' #switch on for physics based fwi       
        np.save(filen, vs_grad)  #switch on physics based fwi
        
        filen = './marmousiSEAM9July/rhooS2EG' + str(epoch1) + '.npy' #switch on for physics based fwi       
        np.save(filen, rho_grad)  #switch on physics based fwi
