...
class Auto23Model(BaseModel):
    ...
    def backward_G11(self, epoch1, batch, lstart):
        ...
        #print("shape of grad :", np.shape(self.grad))   
        if (epoch1 % 10 == 0): 
            filen = './marmousi23/GradAD' + str(batch)+'ep'+str(epoch1)+'.npy' #switch on for physics based fwi       
            np.save(filen, self.grad.cpu().detach().numpy())  #switch on physics based fwi
        
            filen = './marmousi23/FakeAD' + str(batch)+'ep'+str(epoch1)+'.npy' #switch on for physics based fwi       
            np.save(filen, self.fake_B.cpu().detach().numpy())  #switch on physics based fwi
            
            filen = './marmousi23/RealAD' + str(batch)+'ep'+str(epoch1)+'.npy' #switch on for physics based fwi       
