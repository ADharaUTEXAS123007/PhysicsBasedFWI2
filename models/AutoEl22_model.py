...
class AutoEl22Model(BaseModel):
    ...
    def forward(self,epoch1,lstart):
        ...
        [self.fake_B,self.grad,self.latent,self.vp_grad,self.vs_grad,self.rho_grad] = self.netG(self.real_B,self.real_A,lstart,epoch1,self.latent,self.real_C)  # G(A)
        ...
        self.fake_Rho = torch.unsqueeze(self.fake_B[:,2,:,:],1)
        
        self.vp_grad = torch.unsqueeze(self.vp_grad,1)
        self.vs_grad = torch.unsqueeze(self.vs_grad,1)
        self.rho_grad = torch.unsqueeze(self.rho_grad,1)
