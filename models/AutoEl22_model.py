...
class AutoEl22Model(BaseModel):
    ...
    def forward(self,epoch1,lstart):
        ...
        [self.fake_B,self.grad,self.latent,self.vp_grad,self.vs_grad,self.rho_grad,self.loss_D_MSE] = self.netG(self.real_B,self.real_A,lstart,epoch1,self.latent,self.real_C)  # G(A)
        ...
        self.fake_Vp = torch.unsqueeze(self.real_B[:,0,:,:],1)
        self.fake_Vs = torch.unsqueeze(self.fake_B[:,0,:,:],1)
