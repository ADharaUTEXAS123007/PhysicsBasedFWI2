...
class AutoRealDataModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm='batch', netG='AutoRealData',
    ...
    def backward_G11(self, epoch1, batch, lstart, initerror, currenterror, idx):
        ...
        if (epoch1 % 10 == 0): 
            ...
            filen = './marmousiSEAM9July/Grad2JanAD' + str(batch)+'ep'+str(epoch1)+'.npy' #switch on for physics based fwi       
            np.save(filen, self.grad.cpu().detach().numpy())  #switch on physics based fwi
            ...
            filen = './marmousiSEAM9July/Fake2JanAD' + str(batch)+'ep'+str(epoch1)+'.npy' #switch on for physics based fwi       
            np.save(filen, self.fake_B.cpu().detach().numpy())  #switch on physics based fwi
            ...
            filen = './marmousiSEAM9July/Real2JanAD' + str(batch)+'ep'+str(epoch1)+'.npy' #switch on for physics based fwi       
