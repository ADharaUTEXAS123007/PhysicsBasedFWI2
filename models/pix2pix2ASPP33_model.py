...
class Pix2Pix2ASPP33Model(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm='batch', netG='ASPP', dataset_mode='unaligned233', ngf='32')
        if is_train:
    ...
    def forward(self):
        ...
        self.fake_DRMS = self.fake_B[1,2,:,:]
        self.real_DRMS = self.real_B[1,2,:,:]
        self.fake_DRMS = torch.unsqueeze(self.fake_DRMS,0)
        self.real_DRMS = torch.unsqueeze(self.real_DRMS,0)
        self.fake_DRMS = torch.unsqueeze(self.fake_DRMS,0)
        self.real_DRMS = torch.unsqueeze(self.real_DRMS,0)
        #print("---shape fake_B----")
    ...
    def forwardT(self):
        ...
        self.fake_DRMST = self.fake_BT[:,2,:,:]
        self.fake_DRMST = torch.unsqueeze(self.fake_DRMST,0)
        self.real_BT = self.real_B
        self.real_DRMST = self.real_B[:,2,:,:]
        self.real_DRMST = torch.unsqueeze(self.real_DRMST,0)
        #print(np.shape(self.real_DRMST))
        #print("---shape fake_BT----")
        #print(np.shape(self.fake_BT))
        #self.real_BT = self.real_B
