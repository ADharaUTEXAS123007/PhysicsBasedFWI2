...
class Auto26Model(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm='batch', netG='Auto26',
    ...
    def backward_G11(self, epoch1, batch, lstart):
        ...
        #print("shape of grad :", np.shape(self.grad))   
        if (epoch1 % 10 == 0): 
            filen = './marmousi26/GradIntAD' + str(batch)+'ep'+str(epoch1)+'.npy' #switch on for physics based fwi       
            np.save(filen, self.grad.cpu().detach().numpy())  #switch on physics based fwi
        
            filen = './marmousi26/FakeIntAD' + str(batch)+'ep'+str(epoch1)+'.npy' #switch on for physics based fwi       
            np.save(filen, self.fake_B.cpu().detach().numpy())  #switch on physics based fwi
            
            filen = './marmousi26/RealIntAD' + str(batch)+'ep'+str(epoch1)+'.npy' #switch on for physics based fwi       
            np.save(filen, self.real_B.cpu().detach().numpy())  #switch on physics based fwi
            
            #filen = './marmousi25/Down3IntAD' + str(batch)+'ep'+str(epoch1)+'.npy' #switch on for physics based fwi       
            #np.save(filen, self.down3.cpu().detach().numpy())  #switch on physics based fwi
            
            #filen = './marmousi25/Up2IntAD' + str(batch)+'ep'+str(epoch1)+'.npy' #switch on for physics based fwi       
            #np.save(filen, self.up2.cpu().detach().numpy())  #switch on physics based fwi
            
            #filen = './marmousi25/Up1IntAD' + str(batch)+'ep'+str(epoch1)+'.npy' #switch on for physics based fwi       
